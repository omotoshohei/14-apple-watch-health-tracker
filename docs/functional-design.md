...
本システムは、ユーザーがローカル環境でPython CLIを実行し、Apple Healthの `export.xml` または前処理済みCSVを入力として、月次統計サマリーとHTMLレポートを自動生成するスタンドアロンツールです。複数月分のCSVを集計して、長期推移を確認するトレンドHTMLレポートも生成できます。

```mermaid
graph TD
    User[ユーザー]
    CLI[src/cli/*.py]
    Core[src/health_report/]
    XML[export.xml]
    CSV[前処理済みCSV]
    Matplotlib[Matplotlib / SVGグラフ生成]
    ...
    Core -->|読み込み| XML
    Core -->|読み込み/書き出し| CSV
    Core -->|描画指示| Matplotlib
    Core -->|日別/月次データと目標値| Stats
    Stats -->|統計サマリー| Core
    Core -->|SVGとデータ埋め込み| HTMLTemplate
...
| グラフ生成 | `matplotlib` | 日別の棒グラフ、複数月の折れ線グラフ、目標ラインの描画を柔軟にカスタマイズし、HTMLへ埋め込めるSVGとして出力するため。 |
...
| **Sleep Onset Time** | `HKCategoryValueSleepAnalysis` | なし | 主な睡眠セッションの開始時刻（hour、日付跨ぎは24時間加算） |
| **Wake Time** | `HKCategoryValueSleepAnalysis` | なし | 主な睡眠セッションの終了時刻（hour、日付跨ぎは24時間加算） |
| **Awake Count** | `HKCategoryValueSleepAnalysis` | なし | 睡眠中の覚醒区間数 |
| **Awake Duration** | `HKCategoryValueSleepAnalysis` | なし | 睡眠中の覚醒時間合計（minute） |
| **Longest Awake Duration** | `HKCategoryValueSleepAnalysis` | なし | 睡眠中の最長覚醒区間（minute） |
| **First Morning Awake Time** | `HKCategoryValueSleepAnalysis` | なし | 朝の最初の覚醒時刻（hour、日付跨ぎは24時間加算） |
...
sequenceDiagram
    autonumber
    actor User as ユーザー
    participant CLI as src/cli/health_monthly_report.py
    participant Core as src/health_report/
    participant Files as ローカルファイルシステム

    User->>CLI: CLI実行 (year, month, input指定)
    CLI->>Core: レポート生成を依頼
    Core->>Files: XMLまたはCSVの存在確認 & ロード
    Files-->>Core: 入力データ
    Core->>Core: XMLパースまたはCSVロード & 指定年月のデータ抽出
    Core->>Core: 指標ごとに日別集計 (欠損日の判定)
    
    loop 11指標分繰り返し
        Core->>Core: SVGグラフ & 目標ラインの描画 (Matplotlib)
        Core->>Core: 統計サマリー (Average、Maximum、Minimum、Goal Achieved Rate) を算出
    end

    Core->>Core: HTMLテンプレートにSVGと統計サマリーを埋め込み
    Core->>Files: HTMLレポートを output/ 配下に書き出し
    Core-->>CLI: 出力ファイルパス
    CLI-->>User: 完了メッセージ表示
```
...
4. **統計計算時の除外**:
   ...
   - `目標達成率 = (目標を満たした有効な日数) / (有効な日数) * 100 (%)`
   - `lower_is_better=True` の指標は目標値以下を達成、その他の指標は目標値以上を達成として扱います。

### 2. 週次集計とトレンド処理
結合済みの日別CSV (`health_metrics_all.csv`) から週次トレンド用の集計CSVを生成する場合、月曜始まりの完全週だけを対象にします。

1. **端数週の除外**:
   データ範囲の先頭・末尾にある7日未満の週は除外します。
2. **採用週の欠損保持**:
   採用対象の週は月曜から日曜までの7日に再索引し、日付行が存在しない日や指標値が欠けた日は `missing_days` として数えます。
3. **週次統計**:
   週・指標ごとに Average、Maximum、Minimum、Goal Achieved Rate を算出し、平均・最大・最小・達成率の母数から欠損日は除外します。
4. **週次HTML**:
   `health_metrics_weekly.csv` を入力に、11指標それぞれの週次平均折れ線グラフ、目標ライン、週別テーブルを持つHTMLスライドを生成します。
...
- Maximum: 有効日の最大値
- Minimum: 有効日の最小値
- Goal Achieved Rate: 目標を満たした有効日数 / 有効日数 * 100
...
- **レイアウト方式**: スライドを縦方向に並べる（11指標分）。CSSのスクロールスナップとキーボード操作で閲覧できます。
...
- **左側（グラフ領域）**:
  - HTML内にSVGグラフを埋め込み表示。
  - 枠線や背景をHTML全体のテーマに馴染むようにMatplotlib側で透過・調整する。
- **下部（統計サマリー領域）**:
...
| エラー種別 | 処理内容 | ユーザーへの表示 |
|-----------|------|-----------------|
| 入力XML/CSVが存在しない | 処理を中断する | `Error: CSV/XML file not found: ...` |
